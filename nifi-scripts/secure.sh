...
info 'Configuring environment with SSL settings'

: "${KEYSTORE_PATH:?"Must specify an absolute path to the keystore being used."}"
if [ ! -f "${KEYSTORE_PATH}" ]; then
    error "Keystore file specified (${KEYSTORE_PATH}) does not exist."
    exit 1
fi
...
if [ ! -f "${TRUSTSTORE_PATH}" ]; then
    error "Keystore file specified (${TRUSTSTORE_PATH}) does not exist."
    exit 1
fi
...
if [ -z "${NIFI_WEB_PROXY_HOST}" ]; then
    info 'NIFI_WEB_PROXY_HOST was not set but NiFi is configured to run in a secure mode.  The NiFi UI may be inaccessible if using port mapping.'
else
